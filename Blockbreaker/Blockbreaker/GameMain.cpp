...
static int CursorX, CursorY;//カーソル位置

static int Flag_Pause;			//ポーズ状態かどうか(ゲームの進行がすべて停止します)(TRUEでポーズ)
static int Flag_pauseRequest;	//ポーズ状態かどうかのリクエスト(ポーズ処理はすべての処理の最後で行うため)
...
//カーソル位置を相対的に移動する
int GameMain_CursorX_add(int Val) {
	//仮に移動したことにする
	int t = CursorX + Val;
...
//カーソル位置を相対的に移動する
int GameMain_CursorY_add(int Val) {
	//仮に移動したことにする
	int t = CursorY + Val;
	//画面外に出る場合は端で止まるようにValの値を調整する
...
//カーソル位置を取得する
int GameMain_getCursorX() {
	return CursorX;
}

//カーソル位置を取得する
int GameMain_getCursorY() {
	return CursorY;
}

//ブロックの座標？からインゲームの座標の左端を取得する(関数的に出すため、存在しないはずのブロック位置も計算出来ます)
void GameMain_Convert_Block_FromIngame(int blockX, int blockY, double *IngameX, double *IngameY) {
	if (IngameX != NULL) {
...
//ポーズ状態のリクエスト
void GameMain_PauseRequest(int b_Flag) {
